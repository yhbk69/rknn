...
def dfl(position):
    ...
    YOLOv8/v11 使用 DFL 来表示边界框的位置分布，这里将分布预测转换为具体的坐标偏移量
...
def post_process(input_data):
    ...
    # Python 忽略 score_sum 输出，只处理位置和类别
    for i in range(defualt_branch):
        # 对每个检测头的位置输出进行解码
        boxes.append(box_process(input_data[pair_per_branch * i]))
        # 提取类别概率
        classes_conf.append(input_data[pair_per_branch * i + 1])
        # 目标置信度用全 1 替代（因为优化后的模型已经移除了这一分支）
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', type=str, default='rk3566', help='目标 RKNPU 平台，如 rk3566、rk3588 等')
    ...
    for i in range(len(img_list)):
        ...
        # Letterbox 预处理：保持宽高比的缩放 + 填充
        # 由于 RGA 初始化填充色为 (0,0,0)，这里使用 (0,0,0) 而不是 (114, 114, 114)
        pad_color = (0, 0, 0)
        img = co_helper.letter_box(im=img_src.copy(), new_shape=(IMG_SIZE[1], IMG_SIZE[0]), pad_color=(0, 0, 0))
        # BGR 转 RGB
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

        # 如果不是 RKNN 模型，需要进行额外的预处理
        if platform in ['pytorch', 'onnx']:
            ...
            # RKNN 模型已经在内部处理了归一化（通过 mean_values 和 std_values）
            input_data = img
            ...
            if boxes is not None:
                for i in range(boxes.shape[0]):
                    co_helper.add_single_record(
                        image_id=int(img_name.split('.')[0]),  # 图片 ID（去掉扩展名）
                        category_id=coco_id_list[int(classes[i])],  # COCO 类别 ID
                        bbox=boxes[i],  # 检测框坐标
                        score=round(scores[i], 5).item()  # 分数（保留 5 位小数）
                    )
...
_sep = os.path.sep
realpath = realpath.split(_sep)
# Find rknn_model_zoo in path, fallback to parent directory if not found
try:
    zoo_idx = realpath.index('rknn_model_zoo-v2.3.0')
    sys.path.append(os.path.join(realpath[0]+_sep, *realpath[1:zoo_idx+1]))
except ValueError:
    # If not found, add parent directory
    sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(realpath))))
